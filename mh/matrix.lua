...
-- Add matrix by a number
-- num may be of type 'number' or 'complex number'
-- strings get converted to complex number, if that fails then to symbol
function matrix.addnum( m1, num, row ,col )
	local row = row or 0
	local col = col or 0
	local mtx = {}
	-- divide elements by number
	for i = 1,#m1 do
		...
		mtx[i] = mtxi
		for j = 1,#m1[1] do
			if (col==0 or col==j) and (row==0 or row==i) then
				mtxi[j] = m1[i][j] + num else mtxi[j] = m1[i][j] end
		end
	end
	return setmetatable( mtx, matrix_meta )
end
...
--//////////////////////////////
--- @section Object Utility Functions //
--//////////////////////////////
...
---
--@section Functions not changing the matrix
...
--///////////////////////////////
--- @section matrix 'vector' functions //
--///////////////////////////////
